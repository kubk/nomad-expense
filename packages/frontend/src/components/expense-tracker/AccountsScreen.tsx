    ...
    <div className="px-4 mt-4">
      <Card className="bg-gray-900 text-white border-0">
        <CardContent className="p-4">
          <p className="text-gray-400 text-sm">Total Expenses (All Accounts)</p>
          <p className="text-3xl font-bold mt-1">
            ${transactions.reduce((sum, t) => sum + t.usd, 0).toFixed(2)}
          </p>
          <p className="text-gray-400 text-xs mt-2">
            Converted to USD • All time
          </p>
        </CardContent>
      </Card>
    </div>
    ...
    <div className="px-4 mt-4">
      ...
      <div className="space-y-3">
        {accounts.map((account) => {
          ...
          const accountTotal = accountTransactions.reduce(
            (sum, t) => sum + t.usd,
            0,
          );
