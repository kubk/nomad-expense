...
### Minimize Prop Drilling and Rerenders

Access MobX stores directly in components where the state is needed, rather than passing state through props. This reduces unnecessary rerenders and keeps components cleaner.

**Instead of this (prop drilling):**
```tsx
// Parent component
export function Parent() {
  return <Child isOpen={uiStore.isOpen} onClose={uiStore.close} />;
}

// Child component
export function Child({ isOpen, onClose }) {
  return <div className={isOpen ? "open" : "closed"} onClick={onClose} />;
}
```

**Do this (direct store access):**
```tsx
// Parent component  
export function Parent() {
  return <Child />;
...
// Child component - access store directly where needed
...
This pattern:
- Reduces rerenders by accessing state as late as possible
- Eliminates prop drilling 
- Makes components more self-contained
- Leverages MobX's fine-grained reactivity
